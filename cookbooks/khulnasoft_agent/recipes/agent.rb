...
include_recipe 'khulnasoft_agent::repository'

case node['platform']
when 'debian', 'ubuntu'
  apt_package 'khulnasoft-agent' do
    version "#{node['khulnasoft']['patch_version']}-1"
  end
when 'redhat', 'centos', 'amazon', 'fedora', 'oracle'
  if node['platform_version'] >= '8'
    dnf_package 'khulnasoft-agent' do
      version "#{node['khulnasoft']['patch_version']}"
    end
  else
    yum_package 'khulnasoft-agent' do
      version "#{node['khulnasoft']['patch_version']}"
    end
  end
when 'opensuseleap', 'suse' 
  zypper_package 'khulnasoft-agent' do
    version "#{node['khulnasoft']['patch_version']}"
  end
else
...
if agent_auth['khulnasoft_directory']
  args << ' -D ' + agent_auth['khulnasoft_directory']
end
...
execute 'khulnasoft agent auth' do
...
include_recipe 'khulnasoft_agent::common'
